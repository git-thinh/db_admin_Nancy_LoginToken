namespace Nancy.Conventions
{
    ...
    public class StaticContentConventionBuilder
    {
        ...
        public static Func<NancyContext, string, Response> AddDirectory(string requestedPath, string contentPath = null, params string[] allowedExtensions)
        {
            return (ctx, root) =>
            {
                //string url = HttpUtility.UrlDecode(ctx.Request.Url.ToString());
                ...
                if (string.IsNullOrEmpty(fileName))
                    return null;
                if (path.Contains("/.") == false && path.StartsWith(".") == false)
                    return null;
                
                var pathWithoutFilename = GetPathWithoutFilename(fileName, path);
                requestedPath = "/" + pathWithoutFilename.Split('/')[1];
                
                string pathFull = (root + @"\views" + path).Replace("/", @"\").Replace(@"\\", @"\");

                //?????????????????????????????????????????????????????????????
                return new GenericFileResponse(pathFull, ctx);
            };
        }
