namespace Nancy
{
    ...
    public class Request : IDisposable
    {
        ...
        public string PathItemFirst
        {
            get
            {
                string p = Path.Substring(1).Split('/')[0];
                return p == "" ? this.Url.HostName : p;
            }
        }

        public string SiteMap1
        {
            get
            {
                string s = "";
                if (Path != "" && Path != "/")
                {
                    string[] a = Path.Substring(1).Split('?')[0].Split('/');
                    string pi = "";
                    foreach (var i in a)
                    {
                        pi = pi + "/" + i;
                        if (s == "")
                            s = @"<em>/</em><a href=""" + pi + @""">" + i + "</a>";
                        else
                            s = s + @"<em>/</em><a href=""" + pi + @""">" + i + "</a>";
                    }
                    s = @"<em>//</em><a href=""http://" + this.Url.HostName + @""">" + this.Url.HostName + "</a>" + s;
                }
                return s;
            }
        }
